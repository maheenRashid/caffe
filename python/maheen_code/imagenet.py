...
def readSynsetsFile(mapping_file):
...
def removeImagesFromListByClass(im_list_file,mapping_file,classes_to_remove):
    ...
    print min(class_ids),max(class_ids)
    class_id_to_remove,mappings=getImagenetIdToTrainingIdMapping(mapping_file,classes_to_remove);
    idx_to_keep=np.where(np.in1d(class_ids,class_id_to_remove)==0)[0];
    print len(mappings),len(idx_to_keep),len(class_ids),len(set(class_ids));
...
def getMappingInfo(list_files,synset_words,val_gt_file,remove_path=True):
    if remove_path:
        list_files_new=[];
        for file_curr in list_files:
            if '/' in file_curr :
                file_curr=file_curr[file_curr.rindex('/')+1:];
            list_files_new.append(file_curr);
